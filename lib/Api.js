						...
						//console.log('parsing: ' + result["response"]);
							...
							if(result["response"] && result["response"]["status"]) {
								if(result["response"]["status"][0]) {
									console.log('SERVER ' + server + ' ERROR. Return!' + result["response"]["status"][0]);
									
									if(server == "e-api02.verisure.com") {
										ref._BASE_HOST = "e-api01.verisure.com";
										console.log('setting new host to 01');
										Homey.ManagerSettings.set('base_host', "e-api01.verisure.com");
										return;
									}
								}
								else {
